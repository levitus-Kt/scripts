...
# Start
# ---------------------------------------------------\
echo "${YELLOW}Вас приветствует программа по сохранению видеозаписей на резервный диск v2.3.4${NC}"
a=0
b=0
n=0
...
then
	echo ""
	Error
	echo "${CYAN}${BOLD}Вы забыли подключить диск для резервных копий${NC}"
	exit
fi
